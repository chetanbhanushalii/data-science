...
state = graph.invoke({"messages": [{"role": "user", "content": "What is the total price of AAPL and AMZN stock right now?"}]}, config=config)
print("Bot:", state["messages"][-1].content)


state = graph.invoke({"messages": [{"role": "user", "content": "i want to buy 10 RIL stocks and add the total cost to previous cost?"}]}, config=config)
print("Bot:", state["messages"][-1].content)